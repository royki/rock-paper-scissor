...
class GameModes {
  ...
  // Game Mode
  def gameModes: Int => Unit =
    i =>
      i match {
        case 1 =>
          println(
            s"${Console.RED}Enter Number of times Computers'll play together${Console.RESET}"
          )
        ...
        case _ =>
          println(s"${Console.RED}Wrong Choice, Start Again${Console.RESET}")
      }
}
